...
const KnowledgeBase: React.FC = () => {
  ...
  if (!savedPoints.length) {
    return (
      <div className="w-full h-[400px] flex items-center justify-center bg-gray-50 rounded-lg border border-dashed border-gray-300">
    ...
    <div className="w-full space-y-4 fade-in">
      <div className="flex justify-between items-center">
        <div className="flex items-center space-x-2">
          ...
          <h3 className="text-lg font-medium">Chat with Your Knowledge</h3>
        </div>
          ...
          <ScrollArea className="h-[400px] rounded-md border">
            <div className="p-4 space-y-3">
              {savedPoints.map((point) => (
                <Card key={point.id} className="cursor-pointer hover:border-blue-200 transition-colors">
                  <CardHeader className="p-3 pb-0">
                    <CardTitle className="text-sm font-medium line-clamp-1">{point.title}</CardTitle>
                  </CardHeader>
                  <CardContent className="p-3 pt-1">
                    <p className="text-xs text-gray-500 line-clamp-2">{point.content}</p>
                  </CardContent>
                  {point.timestamp && (
                ...
                {chatMessages.length === 0 && (
                  <div className="h-60 flex items-center justify-center text-center">
                    <div className="space-y-2">
                      ...
                      <p className="text-gray-500">Ask a question to start chatting</p>
                    </div>
                  </div>
                )}
